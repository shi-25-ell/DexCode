...
1. **只读模式**：只有确定为只读的操作以及已经加入白名单的命令可以自动执行，其余 agent 副作用需要用户批准。
2. **白名单模式**：保留当前主要体验，工作区内文件修改可自动执行，命令只有命中白名单才自动执行。
...
白名单在只读模式和白名单模式中都生效。两者的主要区别是文件写入和 patch 是否需要逐次批准。完全访问只是跳过普通批准，不绕过以下保护：
...
8. **配置损坏时不扩大权限**：损坏或非法的全局配置不得回退到完全访问或白名单模式。
...
只读模式不能把“看起来像检查”的命令自动当成只读。`npm run lint`、`npx eslint`、`npx tsc` 都可能执行仓库控制的脚本、插件或配置代码，应视为普通执行命令，除非用户把精确命令加入白名单。
...
- **只读模式**：读取和可信只读命令自动执行；写入及其他命令需要批准。
- **白名单模式**：文件修改自动执行；未在白名单中的命令需要批准。
- **完全访问**：所有 agent 操作自动执行；仅保留系统硬性保护。

交互要求：

1. 初次进入页面从 GET 接口读取真实全局状态，不使用 localStorage 作为权威值。
2. 选择只读或白名单模式后立即 PUT；保存中禁用重复提交。
...
- 只读模式：read 直接执行，write/patch/普通命令请求批准。
- 白名单模式：write/patch 直接执行，未命中命令请求批准。
...
1. 以白名单模式启动，确认原有文件编辑体验不变。
2. 在 active track 中切到只读模式，确认下一次写入出现批准卡。
3. 同一 track 切到完全访问，确认下一次尚未授权写入或命令不再弹批准卡。
4. 在只读模式批准一个命令并加入白名单，确认同一 track 再次执行时无需批准。
...
控制：页面分别标注“全局批准模式”和“当前项目命令白名单”；无 workspace 时禁用后者。
