...
const refreshBtn = document.querySelector('#refreshToolsBtn');
let cache = [];
const TEST_PRESETS = {
    read_file: '{\n  "path": "package.json"\n}',
    write_file: '{\n  "path": "test.txt",\n  "content": "hello"\n}',
...
async function loadTools() {
    try {
        ...
        showToast({ kind: 'error', title: '工具列表加载失败', message: error.message });
    }
}
function renderCards() {
    ...
    `;
        card.querySelector('.tool-toggle').addEventListener('click', () => toggleEnabled(tool.name, !tool.enabled));
        card.querySelector('.tool-test-btn')?.addEventListener('click', () => openTestDialog(tool.name));
        card.querySelector('.tool-logs-btn')?.addEventListener('click', () => openLogsDialog(tool.name));
        list.appendChild(card);
    });
}
async function toggleEnabled(toolName, enabled) {
    try {
...
function openTestDialog(toolName) {
  ...
  `;
    dialog.querySelector('.tool-test-args').value = preset;
    ...
    dialog.querySelector('[data-role="run"]').addEventListener('click', async () => {
        ...
        try {
            ...
            resultEl.textContent = `JSON 解析错误:${err.message}`;
            return;
        }
...
async function openLogsDialog(toolName) {
    let logs = [];
    try {
        const data = await fetchJson(`/api/tools/${encodeURIComponent(toolName)}/logs?limit=20`);
        logs = data.logs ?? [];
    }
    catch (error) {
        showToast({ kind: 'error', title: '日志加载失败', message: error.message });
        return;
    }
    const dialog = document.createElement('div');
    dialog.className = 'tree-dialog-overlay visible';
    ...
    <div class="tree-dialog tool-logs-dialog">
      ...
      <div class="tool-logs-list">${logs.length === 0
