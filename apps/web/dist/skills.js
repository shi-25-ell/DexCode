...
const status = document.querySelector('#skillStatus');
const refreshBtn = document.querySelector('#refreshSkillsBtn');
const importBtn = document.querySelector('#importSkillBtn');
const overlay = document.querySelector('#skillImportOverlay');
const pathField = document.querySelector('#skillImportPathField');
const nameField = document.querySelector('#skillImportNameField');
const contentField = document.querySelector('#skillImportContentField');
const pathInput = document.querySelector('#skillImportPathInput');
const nameInput = document.querySelector('#skillImportNameInput');
const contentInput = document.querySelector('#skillImportContentInput');
const reportEl = document.querySelector('#skillImportReport');
const previewBtn = document.querySelector('#skillImportPreviewBtn');
const confirmBtn = document.querySelector('#skillImportConfirmBtn');
const closeBtn = document.querySelector('#skillImportCloseBtn');
let cache = [];
...
async function loadSkills() {
    try {
        ...
        showToast({ kind: 'error', title: 'Skill 加载失败', message: error.message });
    }
}
...
async function toggleEnabled(skillName, enabled) {
    try {
        ...
        showToast({ kind: 'error', title: '操作失败', message: error.message });
    }
}
...
async function previewImport() {
    ...
    try {
        const data = await fetchJson('/api/skills/import/preview', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        reportEl.textContent = `预检查失败:${error.message}`;
    }
    finally {
...
async function confirmImport() {
    ...
    try {
        ...
        showToast({ kind: 'error', title: '导入失败', message: error.message });
    }
    finally {
