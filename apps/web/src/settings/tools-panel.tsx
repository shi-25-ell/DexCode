...
export const TOOL_TEST_PRESETS: Record<string, Record<string, unknown>> = {
  find: { pattern: '*.json' },
  ls: { path: '.' },
  ...
  write_file: { path: '.dexcode-tool-test.txt', content: 'DexCode tool test\n' },
  read_command_output: { task_id: 'command-id' },
  stop_command: { task_id: 'command-id' },
};
...
export function ToolsPanel({ workspaceRef }: { workspaceRef: string }) {
  ...
  const openTest = (tool: ToolInfo) => { setTestTool(tool); setArgsText(JSON.stringify(TOOL_TEST_PRESETS[tool.name] ?? {}, null, 2)); setTestResult(undefined); setTestError(''); };
