...
export function SkillsPanel({ workspaceRef }: { workspaceRef: string }) {
  ...
  const reload = useMutation({
    mutationFn: () => apiJson('/api/skills/reload', { method: 'POST', workspaceRef }),
    onMutate: () => setScanFeedback(''),
    onSuccess: async () => {
      await refresh();
      setScanFeedback(`扫描完成 · ${new Date().toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' })}`);
    },
  });
  ...
  return <><PanelHeader title="Skill" description="查看和管理 Skill；新导入的 Skill 安装到全局目录，可供所有项目使用。" action={<><button className="secondary-button" onClick={() => reload.mutate()} disabled={reload.isPending}><RefreshCw size={15} />{reload.isPending ? '扫描中…' : '重新扫描'}</button><button className="primary-button" aria-label="导入 Skill" onClick={() => setImportOpen(true)}><Download size={15} />导入</button></>} />{reload.error ? <SettingsFeedback error={reload.error} /> : scanFeedback ? <div className="settings-action-feedback" role="status" aria-live="polite">{scanFeedback}</div> : null}<SettingsFeedback loading={query.isLoading} error={query.error ?? toggle.error ?? remove.error} empty={!query.isLoading && query.data?.skills.length === 0 ? '暂无 Skill' : undefined} /><div className="settings-list">{query.data?.skills.map((skill) => <article className="settings-row" key={skill.name}><div className="settings-row-main"><div className="settings-row-title"><strong>{skill.name}</strong><span>{SOURCE_LABELS[skill.source]}</span>{skill.shadowed ? <span>被覆盖</span> : null}</div><p>{skill.description}</p><div className={skill.missingCapabilities.length ? 'settings-warning' : 'settings-meta'}>{skill.missingCapabilities.length ? `缺少能力：${skill.missingCapabilities.join('、')}` : `${skill.allowImplicitInvocation ? '可自动触发' : '仅显式触发'} · 读取 ${skill.usage.readCount} 次 · 激活 ${skill.usage.activationCount} 次`}</div><details><summary>识别详情</summary><div className="skill-detail-grid"><span>标签：{skill.tags.length ? skill.tags.join('、') : '无'}</span><span>文件规则：{skill.filePatterns.length ? skill.filePatterns.join('、') : '无'}</span><span>来源路径：{skill.rootPath}</span></div></details></div><div className="settings-row-actions">{skill.source !== 'builtin' ? <button className="danger-icon" aria-label={`删除 ${skill.name}`} disabled={remove.isPending} onClick={() => { if (window.confirm(`确定删除 Skill“${skill.name}”吗？`)) remove.mutate(skill); }}><Trash2 size={15} /></button> : null}<Toggle enabled={skill.enabled} label={skill.name} disabled={toggle.isPending} onChange={() => toggle.mutate({ name: skill.name, enabled: !skill.enabled })} /></div></article>)}</div>
    {importOpen ? <SettingsDialog title="导入 Skill" onClose={() => setImportOpen(false)}><div className="skill-import-dialog-content"><div className="segmented-control">{([['local_path', '本机目录'], ['workspace_path', '项目目录'], ['inline_markdown', '粘贴内容']] as const).map(([value, label]) => <button type="button" key={value} className={mode === value ? 'active' : ''} onClick={() => changeMode(value)}>{label}</button>)}</div>{mode === 'inline_markdown' ? <><label className="settings-field"><span>Skill 名称</span><input value={name} onChange={(event) => { setName(event.target.value); setReport(null); }} /></label><label className="settings-field"><span>SKILL.md 内容</span><textarea rows={12} value={content} onChange={(event) => { setContent(event.target.value); setReport(null); }} spellCheck={false} /></label></> : <label className="settings-field"><span>{mode === 'local_path' ? '本机绝对路径' : '项目内路径'}</span><input value={path} onChange={(event) => { setPath(event.target.value); setReport(null); }} placeholder={mode === 'local_path' ? 'D:\\skills\\my-skill' : '.aicoding/skills/my-skill'} /></label>}{preview.error ? <p className="dialog-error">{preview.error instanceof Error ? preview.error.message : '预检查失败'}</p> : null}{report ? <section className="skill-import-report"><div><strong>{report.recognized ? '已识别' : '无法识别'}：{report.name || '未命名'}</strong><p>{report.description}</p></div><dl><dt>格式</dt><dd>{report.format}</dd><dt>目标位置</dt><dd>{report.targetPath}</dd><dt>需要能力</dt><dd>{report.requiredCapabilities.join('、') || '无'}</dd><dt>缺失能力</dt><dd>{report.missingCapabilities.join('、') || '无'}</dd><dt>资源</dt><dd>{report.resources.join('、') || '无'}</dd><dt>脚本</dt><dd>{report.scripts.join('、') || '无'}</dd></dl>{report.warnings.length ? <p className="dialog-warning">{report.warnings.join('；')}</p> : null}{report.conflicts.length ? <p className="dialog-error">冲突：{report.conflicts.join('；')}</p> : null}</section> : null}<footer className="dialog-actions"><button className="secondary-button" onClick={() => preview.mutate()} disabled={preview.isPending}>{preview.isPending ? '识别中…' : '预检查'}</button><button className="primary-button" onClick={() => importSkill.mutate()} disabled={!report?.recognized || Boolean(report.conflicts.length) || importSkill.isPending}>{importSkill.isPending ? '导入中…' : '确认导入'}</button></footer></div></SettingsDialog> : null}
  </>;
}
