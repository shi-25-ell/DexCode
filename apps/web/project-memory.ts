...
const editor = document.querySelector<HTMLTextAreaElement>('#projectMemoryEditor')!;
const status = document.querySelector<HTMLElement>('#projectMemoryStatus')!;
const reloadBtn = document.querySelector<HTMLButtonElement>('#reloadProjectMemoryBtn')!;
const saveBtn = document.querySelector<HTMLButtonElement>('#saveProjectMemoryBtn')!;

async function loadProjectMemory() {
  try {
    status.textContent = '加载中…';
    const data = await fetchJson<ProjectMemoryResponse>('/api/project-memory');
