...
const list = document.querySelector<HTMLElement>('#versionList')!;
const status = document.querySelector<HTMLElement>('#versionStatus')!;
const refreshBtn = document.querySelector<HTMLButtonElement>('#refreshVersionsBtn')!;
const createForm = document.querySelector<HTMLFormElement>('#snapshotCreateForm')!;
const nameInput = document.querySelector<HTMLInputElement>('#snapshotNameInput')!;
const descInput = document.querySelector<HTMLInputElement>('#snapshotDescInput')!;

let cache: VersionSnapshot[] = [];

async function loadVersions() {
  try {
    const data = await fetchJson<{ versions?: VersionSnapshot[] }>('/api/versions');
    cache = data.versions ?? [];
    renderVersions();
  } catch (error) {
    status.textContent = '加载失败';
    list.innerHTML = '<div class="version-empty">版本列表加载失败,请稍后重试。</div>';
    showToast({ kind: 'error', title: '快照加载失败', message: (error as Error).message });
  }
}
...
function renderVersions() {
  ...
  cache.forEach((version) => {
    ...
    item.querySelector<HTMLElement>('.version-item-meta')!.textContent = `创建时间:${formatLocalTime(version.createdAt)}`;
    item.querySelector<HTMLButtonElement>('.version-restore-btn')!.addEventListener('click', () => restoreSnapshot(version));
...
createForm.addEventListener('submit', async (e) => {
  ...
  try {
    const data = await fetchJson<{ snapshot?: VersionSnapshot; versions?: VersionSnapshot[] }>('/api/version/snapshot', {
      method: 'POST',
...
async function restoreSnapshot(version: VersionSnapshot) {
  const ok = window.confirm(`确定回滚到 ${version.name || version.id} 吗?\n当前工作区会被覆盖。`);
  if (!ok) return;
  try {
    const data = await fetchJson<{ versions?: VersionSnapshot[]; restoredVersion?: VersionSnapshot }>('/api/version/restore', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
