...
const KNOWN_MODELS: Record<string, KnownModelDescriptor> = {
  'deepseek-v4-flash': { displayName: 'DeepSeek V4 Flash', contextWindow: 1_000_000 },
  'deepseek-v4-pro': { displayName: 'DeepSeek V4 Pro', contextWindow: 1_000_000 },
  'deepseek-v4-flash-vision-exp': { displayName: 'DeepSeek V4 Flash Vision', contextWindow: 1_000_000 },
};
...
export function describeModel(model: string, baseUrl: string): KnownModelDescriptor {
  ...
  if (!known) return { displayName: model };
  ...
  return { displayName: known.displayName };
}
